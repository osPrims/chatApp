...
form.addEventListener("submit", (e) => {
  e.preventDefault();
  if (input.value) {
    if (input.value.substr(0, 3) == "/w ") {
      let msg = input.value.substr(3);
      const idx = msg.indexOf(' ');

      if (idx != -1) {
        const toUsername = msg.substr(0, idx);
        msg = msg.substr(idx + 1);
        socket.emit("chat message", username.value, msg, toUsername);
      }
      else socket.emit("chat message", username.value, msg, "all");
    }
    else socket.emit("chat message", username.value, input.value, "all");
    input.value = "";
  }
...
// Received from a server when a chat message is received
